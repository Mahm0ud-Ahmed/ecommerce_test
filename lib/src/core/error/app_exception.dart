...
class AppException<T> implements Exception {
  ...
  ErrorModel get handleError {
    ...
    if (error is AppException<DioException>) {
      final DioException dioError = (error as AppException<DioException>).error;
      // String typeName = '(${dioError.type.name})\n';
      switch (dioError.type) {
        case DioExceptionType.connectionTimeout:
          errorModel =
              ErrorModel(statusCode: dioError.response?.statusCode, statusMessage: S.current.connect_timeout_message);
          break;
        case DioExceptionType.sendTimeout:
          errorModel = ErrorModel(statusCode: dioError.response?.statusCode, statusMessage: S.current.send_timeout_message);
          break;
        case DioExceptionType.receiveTimeout:
          errorModel =
              ErrorModel(statusCode: dioError.response?.statusCode, statusMessage: S.current.receive_timeout_message);
          break;
        case DioExceptionType.badCertificate:
          errorModel =
              ErrorModel(statusCode: dioError.response?.statusCode, statusMessage: S.current.bad_certificate_message);
          break;
        case DioExceptionType.connectionError:
          errorModel =
              ErrorModel(statusCode: dioError.response?.statusCode, statusMessage: S.current.connection_error_message);
          break;
        case DioExceptionType.cancel:
          errorModel = ErrorModel(statusCode: dioError.response?.statusCode, statusMessage: S.current.cancel_error_message);
          break;
        case DioExceptionType.badResponse:
          if (dioError.response?.statusCode != null && dioError.response != null) {
            if (dioError.response?.data == Map) {
              errorModel = ErrorModel.fromJson(dioError.response?.data);
            } else {
              errorModel = ErrorModel(statusMessage: dioError.message);
            }
            errorModel.statusCode == null
                ? errorModel = errorModel.copyWith(statusCode: dioError.response?.statusCode)
                : null;
          } else {
            String message = getErrorMessage(dioError.response?.statusCode);
    ...
    } else if (error is AppException<HiveError>) {
      final HiveError hiveError = (error as AppException<HiveError>).error;
      ...
      errorModel = ErrorModel(statusMessage: (error as UnsupportedError).message);
    } else {
